...
pub const Instruction = struct {
    ...
    // Bits [0:6]
    pub inline fn opcode(self: Instruction) u7 {
        return @truncate(self.value & 0b1111111);
    }

    // Bits [7:11]
    pub inline fn rd(self: Instruction) u5 {
        return @truncate((self.value >> 7) & 0b11111);
    }

    // Bits [12:14]
    pub inline fn funct3(self: Instruction) u3 {
        return @truncate((self.value >> 12) & 0b111);
    }

    // Bits [15:19]
    pub inline fn rs1(self: Instruction) u5 {
        return @truncate((self.value >> 15) & 0b11111);
    }

    // Bits [20:24]
    pub inline fn rs2(self: Instruction) u5 {
        return @truncate((self.value >> 20) & 0b11111);
    }

    // Bits [25:31]
    pub inline fn funct7(self: Instruction) u7 {
        return @truncate((self.value >> 25) & 0b1111111);
    }

    pub inline fn immIType(self: Instruction) i32 {
        return signExtend((self.value >> 20) & 0xFFF, 12);
    }

    pub inline fn immSType(self: Instruction) i32 {
        const imm0 = (self.value >> 7) & 0b11111;
        const imm1 = (self.value >> 25) & 0b1111111;
        return signExtend((imm1 << 5) | imm0, 12);
    }

    pub inline fn immBType(self: Instruction) i32 {
    ...
    pub inline fn immUType(self: Instruction) i32 {
        const imm: i32 = @bitCast(self.value & 0xFFFFF000);
        return imm >> 12;
    }

    pub inline fn immJType(self: Instruction) i32 {
    ...
    pub inline fn immSystem(self: Instruction) u12 {
        return @truncate((self.value >> 20) & 0b111111111111);
    }

    pub inline fn pred(self: Instruction) u4 {
        return @truncate((self.value >> 24) & 0b1111); // Bits [27:24]
    }

    pub inline fn succ(self: Instruction) u4 {
...
test "opcode extraction" {
    const inst = Instruction{ .value = encode.ADD(1, 2, 3) }; // ADD x1, x2, x3
    try std.testing.expectEqual(0b0110011, inst.opcode());
}

test "rd extraction" {
    const inst = Instruction{ .value = encode.ADD(10, 2, 3) }; // ADD x10, x2, x3
...
test "rs1 extraction" {
    const inst = Instruction{ .value = encode.ADD(1, 11, 3) }; // ADD x1, x11, x3
    try std.testing.expectEqual(11, inst.rs1()); // rs1 = x11
}

test "rs2 extraction" {
    const inst = Instruction{ .value = encode.ADD(1, 2, 12) }; // ADD x1, x2, x12
    try std.testing.expectEqual(12, inst.rs2()); // rs2 = x12
}

test "funct7 extraction" {
    const inst = Instruction{ .value = encode.SUB(1, 2, 3) }; // SUB x1, x2, x3
    try std.testing.expectEqual(0b0100000, inst.funct7());
}

test "I-type immediate extraction" {
    const inst = Instruction{ .value = encode.ADDI(5, 3, 4) }; // ADDI x5, x3, 4
...
test "B-type immediate extraction" {
    const inst = Instruction{ .value = encode.BEQ(2, 3, -8) }; // BEQ x2, x3, -8
    try std.testing.expectEqual(-8, inst.immBType());
}

test "U-type immediate extraction" {
    const inst = Instruction{ .value = encode.LUI(7, 0x12345) }; // LUI x7, 0x12345
    try std.testing.expectEqual(0x12345, inst.immUType());
}

test "J-type immediate extraction" {
    const inst = Instruction{ .value = encode.JAL(8, 0x1000) }; // JAL x8, 0x1000
    try std.testing.expectEqual(0x1000, inst.immJType());
}

test "System immediate extraction" {
    const inst = Instruction{ .value = encode.CSRRC(4, 5, 0x300) }; // CSRRC x4, x5, CSR 0x300
